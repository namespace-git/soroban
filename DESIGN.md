...
## 2026-09-22 Milestone に合わせた調整（v0.2）

- 黄：`--brand` #fbcf52、クリーム `--brand-soft` #fdf3d8、見出しの丸 `--brand-icon` #fad36a（Milestone の画面から実測）
- 塗りのピル：`--profit-pill` #c9f2d6（文字 --profit）／`--loss-pill` #f0605f（白文字）。意味の強い状態（発送してください・取引完了・未着・下書き）だけ塗り、それ以外は薄い地のチップ
- グラフの積み上げ：`--chart-1..6`（赤・橙・黄・緑・水色・薄紫）。仕入先・タグごとに色を割る
- 黄色の帯の見出し（月次の計算書の見出し）
- 人物イラストは使わない（空の状態に小さな絵まで）
